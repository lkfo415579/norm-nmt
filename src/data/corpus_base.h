...
public:
  ...
  Words& front() { return tuple_.front(); }
  const Words& front() const { return tuple_.front(); }
...
public:
  ...
  std::vector<Ptr<CorpusBatch>> split2Corpus(size_t n){
    ...
    std::vector<Ptr<CorpusBatch>> splits;
