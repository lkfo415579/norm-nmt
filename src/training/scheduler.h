...
public:
  ...
  void update(float cost, const std::vector<Ptr<data::Batch>>& batches, Ptr<IMPIWrapper> mpi = nullptr, bool IsLastIter = true) {
    ...
    if (IsLastIter)
      state_->newBatch();
    else if (options_->get<std::string>("disp-freq") == "1")
      update_log = true;
