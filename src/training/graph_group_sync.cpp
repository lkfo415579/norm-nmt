...
SyncGraphGroup::SyncGraphGroup(Ptr<Options> config)
  ...
  // update cycle
  update_cycle_ = options_->get<size_t>("update_cycle");
...
void SyncGraphGroup::update(Ptr<data::Batch> batch) /*override*/ {
  ...
  if(scheduler_) {
    ...
    scheduler_->update(localCost, subBatches, mpi_, islastiter);
