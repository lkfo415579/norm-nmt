...
void Adam::updateImpl(Tensor params, Tensor grads) {
  ...
  if(!mt_) {
    int elements = (int)params->size();
    if (isAMSgrad_){
      alloc_->reserveExact(3 * params->memory()->size());
      alloc_->allocate(maxt_, {1, elements});
      maxt_->set(0.f);
    }
    else
  ...
  // ams grad
  if (isAMSgrad_) {
    // it wrote in a reverse way
    //Element(_1 = min(_2, _3), maxt_, mt_, vt_);
    Element(_1 = min(_2, _3), maxt_, maxt_, vt_);

    // DEBUG
    //std::cout << "maxt:" << maxt_->debug() << std::endl;
    //std::cout << "mt:" << mt_->debug() << std::endl;
    //std::cout << "vt:" << vt_->debug() << std::endl;
    //
...
void Adam::load(const std::string& name,
  ...
  for(auto item : items) {
    ...
    if (isAMSgrad_){
      if(item.name == "adam_maxt") {
        vMaxt.resize(totalSize);
        std::copy(
                (float*)item.data(), (float*)item.data() + totalSize, vMaxt.begin());
      }
    }
  }
  ...
  scatterFn(vMt,
    ...
    if(!opt->mt_ || !opt->vt_) { // lazily allocate
      ...
      if (isAMSgrad_){
        opt->alloc_->allocate(opt->maxt_, {1, (int)size});
      }
    }
  ...
  if (isAMSgrad_){
    scatterFn(vMaxt,
              [&](size_t id,
                ...
                auto opt = std::dynamic_pointer_cast<Adam>(opts[id]);
                opt->maxt_->set(std::vector<float>(begin, end));
              });
  }
...
void Adam::save(const std::string& name,
  ...
  if (isAMSgrad_){
    auto vMAXt = gatherFn([&](size_t localDeviceIndex) {
        auto opt = std::dynamic_pointer_cast<Adam>(opts[localDeviceIndex]);
        std::vector<float> data;
        opt->maxt_->get(data);
        return data;
    ...
    //
    io::Item itemMAXt;
    itemMAXt.name = "adam_maxt";
    itemMAXt.shape = Shape({1, (int)vMAXt.size()});
    itemMAXt.type = Type::float32;
    itemMAXt.bytes.resize(vMAXt.size() * sizeOf(itemMAXt.type));
    std::copy(
            (char*)vMAXt.data(), (char*)vMAXt.data() + vMAXt.size(), itemMAXt.bytes.begin());
    io::saveItems(name, {itemMt, itemVt, itemMAXt});
    return;
  }
