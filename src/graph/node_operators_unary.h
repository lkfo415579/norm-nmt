...
struct ArgMaxNodeOp : public UnaryNodeOp {
    ...
    ArgMaxNodeOp(Expr a, int axis) : UnaryNodeOp(a, newShape(a, axis)) {}

    NodeOps forwardOps() override { return {NodeOp(ArgMax(val_, child(0)->val()))}; }
    ...
    const std::string type() override { return "argmax"; }
};
