...
NodeInitializer DS_init(float depth, float alpha){
  //uniform(-scale, scale)(tensor);
  depth = sqrtf(depth);
  return [depth, alpha](Tensor tensor) {
      float scale = sqrtf(6.0f / (tensor->shape()[-2] + tensor->shape()[-1]));
      scale = scale * alpha / depth;
      tensor->getBackend()->getRandomGenerator()->uniform(tensor, -scale, scale);
  };
}
...
void glorot_normal_scalar(Tensor tensor) {
  float scale = sqrtf(2.0f / tensor->shape()[-1]);
  normal(0.f, scale)(tensor);
}
