...
protected:
  ...
  std::tuple<Expr, Expr> ulrLookup(Ptr<ExpressionGraph> graph,
    ...
    auto uniEmbed = urlEmbeddings[2]; // E(U)  : dimKeys*dimEmb
    auto srcEmbed = urlEmbeddings[3]; // E(I)  : dimQueries*dimEmb
    auto ulrTransform = urlEmbeddings[4]; //A   : dimUlrEmb *dimUlrEmb <-in paper it is fucking beta
