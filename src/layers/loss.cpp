...
Expr LossBase::getCrossEntropy(Expr logits,
                               ...
                               Expr mask,
                               Expr weights) {
  // debug, indices of logits
  //auto getNBestList = createGetNBestListFn(1, logits->shape()[-2], GPU0);
  //auto logPro = logsoftmax(logits);
  //auto max_indices = ArgMax(logPro, -1);
  //max_indices->debug("MaX_INDICES");
  //std::vector<unsigned int> outKeys;
  //std::vector<float> outPathScores;
  //std::vector<size_t> beamSizes(1, 1);
  //getNBestList(beamSizes, logPro->val(), outPathScores, outKeys, true);
  ////logPro->debug("logPro");
...
Expr CrossEntropyRescoreMeanLoss::getCost(Expr logits,
  ...
  auto ce = getCrossEntropy(logits, indices, mask, weights);
  // revo
  //auto test = mean(sum(ce, /*axis =*/ -3), /*axis =*/ -2);
